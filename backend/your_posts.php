...
<?php
	...
	$result = $mysqli->query("SELECT * FROM posts WHERE uid=$Uid ORDER BY id DESC LIMIT 0, 20");
...
<div class="tot-views"><?php echo $row['tot'];?></div>
            ...
            <?php if(!isset($_SESSION['username'])){ /*do nothing*/ }else{?>
<a href="#" id="<?php echo $mes_id;?>" class="delete">x</a>
<?php }?>
...
<?php } ?>
		...
		<nav id="page-nav">
  			<a href="data_yp.php?id=<?php echo $Uid;?>&page=2"></a>
</nav>
<script>
...
$.ajax({
   ...
   $("#freeow").freeow("Deleted..", "This post has been deleted", {
classes: ["smokey"],
autoHide: true
});
  }
