...
if($ThisUserSql = $mysqli->query("SELECT * FROM users WHERE id='$upid'")){
   ...
   $ThisUN = $ThisUserRow['username'];
   
   		$ThisUserLong = $ThisUserRow['about'];
		$ThisUserstrd = strlen ($ThisUserLong);
		if ($strd > 160) {
		$ThisAboutLong = substr($ThisUserLong,0,160).'...';
		}else{
		$ThisAboutLong = $ThisUserLong;}
...
<head>
<meta charset="utf-8">
<title><?php echo ucfirst($ThisUserRow['username']);?> Profile - <?php echo $settings['name']; ?></title>
<meta name="description" content="<?php echo $ThisAboutLong;?>" />
...
$(function() {
$("#contain-2").on("click",".like",function()
{
var id = $(this).attr("id");
var name = $(this).attr("name");
var dataString = 'id='+ id ;
